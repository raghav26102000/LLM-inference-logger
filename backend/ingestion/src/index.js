...
import { proxyRouter } from "./routes/proxy.js";        // NEW
...
// Sliding-window per IP. 60 req/min on /api/ingest, 120/min on /api/proxy/chat.
...
function rateLimit(maxPerMinute) {
  return (req, res, next) => {
    ...
    // Periodic GC — avoid unbounded map growth
    if (Math.random() < 0.01) {
...
try {
  const client = createClient({
    // Reads from env (set by docker-compose / k8s ConfigMap).
...
app.use("/api/proxy",         rateLimit(120), proxyRouter);       // NEW
app.use("/api/conversations", conversationsRouter);
app.use("/api/dashboard",     dashboardRouter);

// ── SSE — live dashboard updates via Redis pub/sub ────────────────────────────
...
app.listen(PORT, () => {
  ...
  console.log(
    configured.length
      ...
      : "   ⚠️  No provider API keys set — add them to .env"
  );
});
